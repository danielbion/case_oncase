...
* Sodium intake below daily recommended;
...
* The more detailed the directions, better is the rating.
...
labels = labels %>% add_count(categoriesUpper) %>% filter(n >= 5) %>% select('categoriesUpper')
...
scene.mod.cc = train(lrncc, task, subset = train.set)
scene.pred.cc = predict(scene.mod.cc, task = task, subset = test.set)
...
scene.mod.br = train(lrnbr, task, subset = train.set)
scene.pred.br = predict(scene.mod.br, task = task, subset = test.set)
